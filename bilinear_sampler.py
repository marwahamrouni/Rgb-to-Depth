...
# Source code https://github.com/alwynmathew/bilinear-sampler-pytorch/blob/master/bilinear_pytorch.py#

import torch
from torch.nn.functional import pad
CUDA_LAUNCH_BLOCKING=1
def apply_disparity(input_images, x_offset, wrap_mode='border', device='cuda:0'):
    ...
    # meshgrid function)
    x = torch.linspace(0, width - 1, width).repeat(height, 1).float().to(device)
    y = torch.linspace(0, height - 1, height).repeat(width, 1).float().transpose(0, 1).to(device)
    ...
    base = dim1 * torch.arange(num_batch).float().to(device)
